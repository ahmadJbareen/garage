    ...
    <form class="res_form" method="post" action="new_user.php">
        ...
        <div class="input-group">
            ...
            value="<?php if(isset($email)) echo $email; ?>"
                pattern="[a-zA-Z0-9._%+-]+@[a-zA-Z.-]+\.[a-zA-Z0-9._%+-]{2,}$"
                title="Email must contain @ in email address, example(user@gmail.com)" required>
        </div>
        <div class="input-group">
            <label>סיסמא:</label>
            <input type="password" name="password_1" placeholder="סיסמא" pattern=".{5,}" title="Six or more characters"
                required>
        </div>
        <div class="input-group">
            <label>אימות סיסמא:</label>
            <input type="password" name="password_2" placeholder="אימות סיסמא" required>
        </div>
