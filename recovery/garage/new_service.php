...
{ // חיפוש לפי רכב
    ...
    $username = mysqli_real_escape_string($con,$_POST['username']);
    $idcar = mysqli_real_escape_string($con,$_POST['idcar']);
    $car_sql=" select * from cars where idcar like '%$idcar%' ";
    $query_car=mysqli_query($con,$car_sql);
    $rowcars=mysqli_fetch_assoc($query_car);
    $valid_car=false;
    if($rowcars > 0)
...
if (isset($_POST['create'])) { // יוצר טיפול
  ...
  if($valid_name == true and $valid_car==true) {
        $sql = "insert into service(username,phone,idcar,km,date,typeservice,text) values ('$username','$phone','$idcar','$km','$date','$typeservice','$text');";
...
<body>
    ...
    <form method="post" action="new_service.php">
        ...
        <div class="input-group">
            ...
            <input type="text" name="phone" placeholder="מספר טלפון" pattern="[0-9]{3}-[0-9]{7}"
