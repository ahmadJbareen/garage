...
if (isset($_POST['btn_addCar'])) { // adding car to user
    ...
    if($valid_name == true and $valid_car==false) {
...
<body>
    ...
    <form class="res_form" method="post" action="new_car.php">
        ...
        <div class="input-group">
            ...
            <input type="text" name="username" placeholder=" שם משתמש" value="<?php if(isset($username)) echo $username; ?>" autocomplete = "off"
            required>
        </div>
        <div class="input-group">
            ...
            <input type="text" name="idcar" placeholder="מספר רכב"  value="<?php if(isset($idcar)) echo $idcar; ?>"
                pattern="[0-9]{3}[-][0-9]{2}[-][0-9]{3}|[0-9]{2}[-][0-9]{3}[-][0-9]{2}|[0-9]{7,8}"
                title="Number of car must 00-000-00 OR 000-00-000" autocomplete = "off"
                required >
        </div>
        <div class="input-group">
            <label>שנת יצור:</label>
            <input type="int" name="year" placeholder="שנת יצור"  value="<?php if(isset($year)) echo $year; ?>"
                pattern="19+[0-9]{2}|20+[0-1]{1}[0-9]{1}|202+[0-2]{1}" 
                title="the year must to be 1900-2022" autocomplete = "off"
            required>
        </div>
        <div class="input-group">
            <label>סוג רכב:</label>
            <input type="text" name="typecar" placeholder="סוג רכב"  value="<?php if(isset($typecar)) echo $typecar; ?>"
             autocomplete = "off"
            required>
        </div>
        <div class="input-group">
            <label>שמן מנוע:</label>
            <input type="text" name="engineoil" placeholder="שמן מנוע" value="<?php if(isset($engineoil)) echo $engineoil; ?>"
             autocomplete = "off"
             required>
        </div>
        <div class="input-group">
            <label>סוג דלק:</label>
            <input type="text" name="typefuel" placeholder="סוג דלק" value="<?php if(isset($typefuel)) echo $typefuel; ?>"
             autocomplete = "off"
             required>
        </div>
        <div class="input-group">
            <label>נוזל קירור:</label>
            <input type="text" name="coolant" placeholder="נוזל קירור" value="<?php if(isset($coolant)) echo $coolant; ?>"
             autocomplete = "off"
            required>
        </div>
        <div class="input-group">
            <label>לחץ אוויר:</label>
            <input type="text" name="airpressure" placeholder="לחץ אוויר" value="<?php if(isset($airpressure)) echo $airpressure; ?>"
            autocomplete = "off"
            required>
        </div>
        <div class="input-group">
            <label>מידת צמיגים:</label>
            <input type="text" name="tiresize" placeholder="מידת צמיגים" value="<?php if(isset($tiresize)) echo $tiresize; ?>"
            autocomplete = "off"
            required>
        </div>
        <div class="input-group">
