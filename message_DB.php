...
if (isset($_POST['search']))// search cars
    ...
    $query=" SELECT * FROM `_message` WHERE CONCAT (`fromU`, `toU`, `phone`, `typeservice`, `email`, `date`, `text`) LIKE '%".$valueToSearch."%' ORDER BY `id` DESC"; 
    $search_result=filterTable($query);
    }  
else{
        $query=" SELECT * FROM `_message` ORDER BY `_message`.`id` DESC LIMIT 10";
        $search_result=filterTable($query); 
    }
